...
// Create Hisse  Arrays
let carArr = ['BMW', 'Mercedes', 'Toyota', 'Chevrolet', 'Nissan', 'Hyundai', 'Lada', 'Opel', 'Audi', 'Bentley', 'Volkswagen', 'Ford', 'Porche']
let models = [
    [114, 116, 118, 120, 'X5', 'X6', "M5", "M550", 'M6', 'Z3'],
    ['A-140', 'A-150', 'Cheska', 'img-190', "AMG-GT", 'S-class', 'GLK'],
    ['Camry', 'Corolla', 'Prius', 'Hightlander', 'Hilux', 'Land-cruser', 'Prado', 'Matrix', 'RAV4', 'Supra', 'Yaris'],
    ['Aveo', 'Cobalt', 'Outlander', 'Niva', 'Land-Cruser', 'Malibu'],
    ['Tiida', 'Sunny', 'Altima', 'Juke', 'Maxima', 'Murano', 'Patfghinder', 'Qasqhai', 'X-trail', 'Teana'],
    ['Elantra', 'Tucson', 'Sonata', 'Accent', 'Azera', 'Santa-Fe', 'Grandeur', 'Terracan'],
    ['Niva', 'Oka', 'Priora', '21-03', '21-04', '21-05', '21-06', '21-07', '21-08', '21-10', '21-11', '21-12', '21-15'],
    ['Astra', 'Vectra', 'Calibra', 'Ampera', 'Omega', 'Zafira', 'Tigra', 'Vita'],
    ['Q-7', 'Q-8', 'Q-9', 'A5-Sportback', 'rs-etron', 'rs-7', 'S-5'],
    ['Arnage', 'Azure', 'Bentayqa', 'Brootslands', 'Continental', 'Flying', 'Mulsanne'],
    ['Amarok', 'Arteon', 'Corrado', 'Craftor', 'Passat', 'Caravelle', 'Jetta', 'Luppo', 'Touareg', 'Tiguan'],
    ['Focus', 'Fusion', 'Fiesta', 'Galaxy', 'Linkoln', 'Mustang', 'Puma', 'Ranger', 'Transit'],
    ['Cayenne', 'Macan', 'Panamera', 'Taycan', 911],
]


let banArr = ['', 'Dartqi', 'Avtobus', 'Furqon', 'Hetcbek', 'Sedan', 'Moped', 'Minivan', 'Mikroavtobus', 'Liftbeck', 'Universal', 'Pikup', 'Motosiklet']
let cityArr = ['', 'Bakı', 'Qəbələ', 'Bərd', 'Səki', 'Şamaxı', 'Mingəçevir', 'Qazax', 'Tovuz', 'Şuşa', 'Naxçıvan', 'Astara', 'Lerik']

let mark = document.getElementById('marka')
...
let modInput = document.getElementsByClassName('m-inp')
...
function getModel() {
    ...
    for (let i = 0; i < carArr.length; i++) {
        for (j = 0; j < models[i].length; j++) {
            if (mark.value == carArr[i]) {
                code += `<option>${models[i][j]}</option>`
            }
        }

    }
    modelName.innerHTML = code
}
...
let val = ''
for (let i = 0; i < 12; i++) {
    elem += `<option>${cityArr[i]}</option>`
    val += `<option>${banArr[i]}</option>`
}
ban.innerHTML = val
town.innerHTML = elem

// İllər
let newDate = new Date().getFullYear() - 120

let yearKod = ''
yearKod = `<option selected></option>`
for (let i = new Date().getFullYear() - 120; i <= new Date().getFullYear(); i++) {
    yearKod += `<option>${i}</option>`
}
year.innerHTML = yearKod
...
function createCar() {
    ...
    for (let i = 0; i < modInput.length; i++) {
        if (modInput[i].value == '' || modInput[i].value.trim == '') return

    }

    saleCars.push(obj)
    console.log(saleCars);
    cards.innerHTML += (saleCars[saleCars.length-1].Barter == true && saleCars[saleCars.length-1].Credit == true) ? `
    <div class="card" style="position: relative;">
        <i class="fa-solid fa-arrows-rotate"></i>
        <i class="fa-solid fa-percent"></i>
        <div style='width:100%;border-top-left-radius:5px;border-top-right-radius:5px; height:200px;background: url(${saleCars[saleCars.length-1].ImageUrl}) center/cover'></div> 
        <div class="cardInfo">
        <h2>${saleCars[saleCars.length-1].Price + " " + saleCars[saleCars.length-1].Valut}</h2>
        <p>${saleCars[saleCars.length-1].Marka.toUpperCase() + " " +saleCars[saleCars.length-1].Model.toUpperCase()} </p>
        <p>${saleCars[saleCars.length-1].Year}, ${saleCars[saleCars.length-1].Motor}, ${saleCars[saleCars.length-1].Kilometrs + " " + "Km"} </p> 
        <p>${saleCars[saleCars.length-1].City.toUpperCase()}</p> 
        </div>
    </div>`
: (saleCars[saleCars.length-1].Barter == true) ? `
    <div class="card" style="position: relative;">
        <i class="fa-solid fa-arrows-rotate"></i>
        <div style='width:100%;border-top-left-radius:5px;border-top-right-radius:5px; height:200px;background: url(${saleCars[saleCars.length-1].ImageUrl}) center/cover'></div> 
        <div class="cardInfo">
...
: (saleCars[saleCars.length-1].Credit == true) ?
`<div class="card" style="position: relative;">
        <i class="fa-solid fa-percent"></i>
        <div style='width:100%;border-top-left-radius:5px;border-top-right-radius:5px; height:200px;background: url(${saleCars[saleCars.length-1].ImageUrl}) center/cover'></div> 
        <div class="cardInfo">
